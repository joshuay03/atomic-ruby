...
class TestAtomicCountDownLatch < Minitest::Test
  def test_with_invalid_count
    assert_raises AtomicRuby::AtomicCountDownLatch::InvalidCountError do
      AtomicRuby::AtomicCountDownLatch.new(0.5)
    end
  end

  def test_count
    latch = AtomicRuby::AtomicCountDownLatch.new(1)
  ...
  def test_count_down_when_already_counted_down
    latch = AtomicRuby::AtomicCountDownLatch.new(1)
    latch.count_down
    assert_raises AtomicRuby::AtomicCountDownLatch::AlreadyCountedDownError do
      latch.count_down
    end
  end
  ...
  def test_wait_with_multiple_threads
    latch = AtomicRuby::AtomicCountDownLatch.new(5)
